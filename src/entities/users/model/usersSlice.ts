...
export const fetchPopularUsers = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const response = await fetch('/db/users.json');
      if (!response.ok) throw new Error('Ошибка загрузки популярных пользователей');
      const data = await response.json();
...
export const fetchRecentUsers = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const response = await fetch('/db/users.json');
      if (!response.ok) throw new Error('Ошибка загрузки недавних пользователей');
      const data = await response.json();
...
export const usersSlice = createSlice({
  ...
  reducers: {
    ...
    updateUser(state, action: PayloadAction<{ id: string; updates: Partial<TUserMain> }>) {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchPopularUsers.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload as string || 'Failed to fetch popular users';
      })
      ...
      .addCase(fetchRecentUsers.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload as string || 'Failed to fetch recent users';
      })
...
export const selectUsersFiltered = createSelector(
  ...
  (users, filters) => {
    return users.filter((user) => {
      // Фильтр по полу
      const matchesGender = filters.gender === null || user.gender === filters.gender;
      
      // Фильтр по городам
      const matchesCity = filters.city.length === 0 || filters.city.includes(user.city);
